...
echo "Imagen: $IMAGE "
echo "-- Eliminar imagen manualmente:"
echo "   podman rmi --force $IMAGE || true"

mkdir -p $SOURCE/dotfiles

mkdir -p "$SOURCE/dotfiles" "$MAIN/pkg" "$SOURCE/backups"
...
podman build -t $IMAGE  -f $SOURCE/init/ContainerfileFedora
